...
export function formatRelativeDate(inputDate?: string | number | Date | null): string {
  if (!inputDate) return "Recently";

  if (typeof inputDate === "string") {
    const trimmed = inputDate.trim();
    if (trimmed.includes("ago") || (isNaN(Number(trimmed)) && trimmed.length <= 15 && !trimmed.includes("-") && !trimmed.includes("T"))) {
      return trimmed;
    }
  }
  ...
  if (!dateVal.isValid()) {
    return typeof inputDate === "string" ? inputDate : "Recently";
  }
